...
class DtoTest {
  ...
  void createResourceTypeRequestDto() throws Exception {
    CreateResourceTypeRequestDto dto = new CreateResourceTypeRequestDto(
  ...
  void deleteResourcesRequestDto() throws Exception {
    IRN irn = IRN.from("irn:rc73dbh7q0:myapp:4atcicnisg::device/dev/7e1edad5-7841-4d38-bdf1-bdc575b0e989");
    DeleteResourcesRequestDto dto = new DeleteResourcesRequestDto(List.of(irn));

    String marshalledJson = objectMapper.writeValueAsString(dto);
    assertThat(marshalledJson).isEqualTo("{\"resourceIDs\":[\"" + irn.toBase64() + "\"]}");
  }

  @Test
  void evaluateResourceTypeRequest() throws Exception {
    EvaluateResourceTypeRequest dto = new EvaluateResourceTypeRequest("read", "test-app", "document", "tenant1");
  ...
  void evaluateResourcesRequest() throws Exception {
    EvaluateResourcesRequest dto = new EvaluateResourcesRequest("read", Collections.singletonList("resource1"));
